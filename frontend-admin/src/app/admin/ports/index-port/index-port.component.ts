...
@Component({
  selector: 'app-index-port',
  standalone: true,
  imports: [SharedModule, RouterLink],
  templateUrl: './index-port.component.html',
  styleUrl: './index-port.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class IndexPortComponent implements OnInit {
  dtOptions: Config = {};
  ports: Port[] = [];
  ...
  private handleAjaxRequest(dataTablesParameters: any, callback: any): void {
    this.portService.getPaginatedData(dataTablesParameters).subscribe(callback);
  }

  private getTableColumns(): any[] {
    return [
      { data: 'portName', title: 'Port name', searchable: true },
      { data: 'countryName', title: 'Country name', searchable: true },
      { data: 'portCode', title: 'Code port', searchable: true },
      { data: 'portLongitude', title: 'Longitude', searchable: true },
      { data: 'portLatitude', title: 'Latitude', searchable: true },
      {
        data: 'portLogo',
        title: 'Logo',
        render: (logo: string, type: any, row: any, meta: any) => {
          const imgId = `logo-img-${meta.row}`;
          this.portService.getPortLogo(logo).subscribe((url) => {
            const imgEl = document.getElementById(imgId) as HTMLImageElement;
            if (imgEl) {
              imgEl.src = url;
            }
          });
          // Initially return a placeholder with a unique ID
          return `<img id="logo-img-${meta.row}"
                    style="width: 40px; height: 40px; border-radius: 50%; object-fit: cover;" alt="Company Logo" />`;
        },
      },
      {
        title: 'Actions',
  ...
  private setupEditButtonListener(row: Node, data: any): void {
    ...
    if (editBtn) {
      this.renderer.listen(editBtn, 'click', () => {
        this.router.navigate(['/edit-ports', data.id]);
      });
    }
  }
  ...
  private handleDeleteSuccess(response: any): void {
    if (response.isSuccess) {
      this.configService.showSuccessAlert('Port supprimé avec succès.');
    } else {
