...
@Component({
  selector: 'app-edit-transportations',
  standalone: true,
  imports: [SharedModule],
  templateUrl: './edit-transportations.component.html',
  styleUrl: './edit-transportations.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class EditTransportationsComponent implements OnInit {
  form!: FormGroup;
  submitted = false;
  ...
  getData() {
    ...
    this.transportationService.show(id).subscribe({
      next: (data: Transportation | null) => {
        if (!data) {
          this.router.navigateByUrl(
            this.configService.ENDPOINTS.transportations
          );
          return;
        }
  ...
  submit(): void {
    ...
    this.transportationService.update(id, this.form.value).subscribe({
