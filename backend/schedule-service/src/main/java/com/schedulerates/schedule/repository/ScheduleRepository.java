...
public interface ScheduleRepository extends JpaRepository<ScheduleEntity, String> {
    ...
    /************************************************************************************************************ */
      @Query("""
        ...
        AND (:companyId IS NULL OR s.companyId = :companyId)
        AND (
                (:startDateSchedule IS NULL AND :endDateSchedule IS NULL)
                OR (:searchOn = '1' AND :startDateSchedule IS NOT NULL AND :endDateSchedule IS NOT NULL AND s.dateDepart BETWEEN :startDateSchedule AND :endDateSchedule)
                OR (:searchOn = '2' AND :startDateSchedule IS NOT NULL AND :endDateSchedule IS NULL AND s.dateDepart = :startDateSchedule)
        )
        """)
        Page<ScheduleEntity> findByScheduleFilters(
                @Param("portFromCode") String portFromCode,
                @Param("portToCode") String portToCode,
                @Param("startDateSchedule") String startDateSchedule,
                @Param("endDateSchedule") String endDateSchedule,
