package com.schedulerates.client.filter;

import com.schedulerates.client.client.UserServiceClient;
import com.schedulerates.client.model.auth.Token;
...
/**
 * Custom filter named {@link CustomBearerTokenAuthenticationFilter} for
 * handling Bearer token authentication in HTTP requests.
...
public class CustomBearerTokenAuthenticationFilter extends OncePerRequestFilter {
     ...
     *
     * <p>
     * Extracts the Bearer token from the Authorization header, validates the token
     * using
     * {@link UserServiceClient#validateToken(String)}, and sets the authentication
     * context
     * if the token is valid. If validation fails, sets the appropriate HTTP status
     * and
     * writes an error message to the response. The filter chain proceeds regardless
     * of
     * token validation success or failure.
     * </p>
     *
