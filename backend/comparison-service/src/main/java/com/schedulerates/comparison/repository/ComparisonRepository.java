...
public interface ComparisonRepository extends JpaRepository<ComparisonEntity, String> {
    ...
    // Get daily Comparison counts by user
        ...
        @Query("""
            SELECT s FROM ComparisonEntity s
            WHERE s.active = '1'
            AND (:portFromCode IS NULL OR s.portFromCode = :portFromCode)
            AND (:portToCode IS NULL OR s.portToCode = :portToCode)
            AND (:transportationId IS NULL OR s.transportationId = :transportationId)
            AND (:containerId IS NULL OR s.containerId = :containerId)
            AND (
                (:startDateComparison IS NULL AND :endDateComparison IS NULL)
                OR (:startDateComparison IS NOT NULL AND :endDateComparison IS NOT NULL AND s.dateDepart BETWEEN :startDateComparison AND :endDateComparison)
                OR (:startDateComparison IS NOT NULL AND :endDateComparison IS NULL AND s.dateDepart = :startDateComparison)
            )
        """)
        Page<ComparisonEntity> findByComparisonFilters(
                @Param("portFromCode") String portFromCode,
                @Param("portToCode") String portToCode,
                @Param("startDateComparison") String startDateComparison,
                @Param("endDateComparison") String endDateComparison,
                @Param("transportationId") String transportationId,
