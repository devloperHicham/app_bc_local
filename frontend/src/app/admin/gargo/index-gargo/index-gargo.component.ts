...
@Component({
  selector: 'app-index-gargo',
  standalone: true,
  imports: [SharedModule, RouterLink],
  templateUrl: './index-gargo.component.html',
  styleUrl: './index-gargo.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class IndexGargoComponent implements OnInit {
  dtOptions: Config = {};
  gargos: Gargo[] = [];
  ...
  private handleAjaxRequest(dataTablesParameters: any, callback: any): void {
    this.gargoService.getPaginatedData(dataTablesParameters)
      .subscribe(callback);
  }

  private getTableColumns(): any[] {
    return [
      { data: 'gargoName', title: 'Désignation', searchable: true},
      {
        title: 'Actions',
        data: null,
        orderable: false,
  ...
  private setupEditButtonListener(row: Node, data: any): void {
    ...
    if (editBtn) {
      this.renderer.listen(editBtn, 'click', () => {
        this.router.navigate(['/edit-gargos', data.id]);
      });
    }
  }
  ...
  private delete(id: string): void {
    this.gargoService.delete(id).subscribe({
  ...
  private handleDeleteSuccess(response: any): void {
    if (response.isSuccess) {
      this.configService.showSuccessAlert('Gargo supprimé avec succès.');
    } else {
