...
export class PortService {
  ...
  constructor(
    ...
    this.url = `${this.configService.URL_API}${this.configService.ENDPOINTS.settings}/ports`;
  }

  getAll(): Observable<Port[]> {
    const urls = `${this.url}/all`;
    return this.http
      .get<ApiResponses<Port[]>>(urls, this.configService.httpOptions)
      .pipe(
  ...
  show(id: string): Observable<Port | null> {
    const url = `${this.url}/${id}`;
    return this.http
      .get<ApiResponses<Port>>(url, this.configService.httpOptions)
      .pipe(
        map((response) => {
  ...
  getPortLogo(filename: string): Observable<string> {
