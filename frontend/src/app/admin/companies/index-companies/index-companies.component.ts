...
@Component({
  ...
  styleUrl: './index-companies.component.scss',
...
export class IndexCompaniesComponent implements OnInit {
  ...
  private getTableColumns(): any[] {
    return [
      { data: 'companyName', title: 'Désignation', searchable: true},
      {
        data: 'companyLogo',
        title: 'Logo',
        render: (logo: string, type: any, row: any, meta: any) => {
          const imgId = `logo-img-${meta.row}`;
          this.companiesService.getCompanyLogo(logo).subscribe((url) => {
  ...
  private setupEditButtonListener(row: Node, data: any): void {
    ...
    if (editBtn) {
      this.renderer.listen(editBtn, 'click', () => {
        this.router.navigate(['/edit-companies', data.id]);
      });
    }
  }
  ...
  private deleteFaq(id: string): void {
  ...
  private handleDeleteSuccess(response: any): void {
    if (response.isSuccess) {
      this.configService.showSuccessAlert('FAQ supprimé avec succès.');
    } else {
