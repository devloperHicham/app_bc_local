  ...
  async pasteFromClipboard() {
    try {
      ...
      for (const line of lines) {
        ...
        const dateDepart = this.formatDate(cols[0]);
        const dateArrive = this.formatDate(cols[1]);
      ...
      this.configService.showErrorAlert(
        'Erreur lors de la lecture du presse-papiers. Vérifiez les permissions.'
      );
      console.error(err);
    }
  }
