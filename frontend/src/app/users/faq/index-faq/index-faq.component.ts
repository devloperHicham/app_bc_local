...
@Component({
  selector: 'app-index-faq',
  standalone: true,
  imports: [SharedModule, RouterLink],
  templateUrl: './index-faq.component.html',
  styleUrl: './index-faq.component.scss',
  schemas: [CUSTOM_ELEMENTS_SCHEMA],
})
export class IndexFaqComponent implements OnInit {
  dtOptions: Config = {};
  faqs: Faq[] = [];
  users: User[] = [];
  ...
  private handleAjaxRequest(dataTablesParameters: any, callback: any): void {
    this.faqService.getPaginatedData(dataTablesParameters).subscribe(callback);
  }
  ...
  private setupEditButtonListener(row: Node, data: any): void {
    ...
    if (editBtn) {
      this.renderer.listen(editBtn, 'click', () => {
        this.router.navigate(['/edit-faqs', data.id]);
      });
    }
  }
